...
namespace Bhp.Mining
{
    ...
    {   
        public MinerTransaction MakeMinerTransaction(Wallet wallet, uint blockIndex, ulong nonce, Fixed8 amount_servicefee,Fixed8 amount_netfee )
        {
            ...
            AddServiceFee(outputs, amount_servicefee);
        ...
        private void AddServiceFee(List<TransactionOutput> outputs, Fixed8 amount_servicefee)
        { 
            if (amount_servicefee != Fixed8.Zero)
            {
                outputs.Add(TransactionFeeOutput(amount_servicefee));
            }
        }
        ...
        private TransactionOutput TransactionFeeOutput(Fixed8 service_netfee)
        {
            return new TransactionOutput
            {
                AssetId = Blockchain.GoverningToken.Hash,
