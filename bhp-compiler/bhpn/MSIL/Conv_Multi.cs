...
namespace Bhp.Compiler.MSIL
{
    ...
    public partial class ModuleConverter
    {
        ...
        */
        ...
        public bool IsNotifyCall(Mono.Cecil.MethodDefinition defs, Mono.Cecil.MethodReference refs, BhpMethod to, out string name)
        {
        ...
        private int _ConvertInitObj(OpCode src, BhpMethod to)
        {
            ...
            _ConvertPush(type.Fields.Count, null, to);//插入个数
            if (type.IsValueType)
            {
        ...
        private int _ConvertStfld(ILMethod method, OpCode src, BhpMethod to)
        {
        ...
        private int _ConvertLdfld(OpCode src, BhpMethod to)
        {
            var field = (src.tokenUnknown as Mono.Cecil.FieldReference).Resolve();
            var type = field.DeclaringType;
            var id = type.Fields.IndexOf(field);
