...
namespace Bhp.UI
{
    public partial class AssetRegisterDialog : Form
    {
        ...
        private void AssetRegisterDialog_Load(object sender, EventArgs e)
        {
            ...
            comboBox2.Items.AddRange(Program.CurrentWallet.GetAccounts().Where(p => !p.WatchOnly && p.Contract.Script.IsStandardContract()).Select(p => p.GetKey().PublicKey).ToArray());
